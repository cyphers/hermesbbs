...
interface
	...
	uses
		Serial, Sound, Types, AppleTalk, ADSP, CommResources, CRMSerialDevices, CTBUtilities, GestaltEqu, serialNums;
	const
		HERMES_VERSION = '2.2';
	...
	var
		...
		writeDirecttoLog, answerCalls, hasGPI, screwUp, gSndCalledBack: boolean;
		myUsers: UListHand;
...
implementation
	...
	function QuickCheckSerial: boolean;
		var
			...
			i: integer;
	begin
		if gSerialBinary <> nil then
			DisposPtr(gSerialBinary);
		serialValid := true;
		if length(intSystRec.realSerial) > 0 then
		begin
			serialValid := false;
			serialTemp := intSystRec.realserial;
			if TrickySerialToBinary(gserialBinary, serialTemp) then
				if TrickyQuickValidateSerial(gserialBinary) then
					serialValid := true;
			if not serialValid then
				intSystRec.realSerial := '';
		end;
		for i := 1 to intSystRec.numNodes do
		begin
			theNodes[i]^.serialBinary := gserialBinary;
			theNodes[i]^.mySystNode := @intSystRec;
		end;
		QuickCheckSerial := serialValid;
	end;
