...
interface
	uses
		AppleTalk, ADSP, Serial, Sound, SANE, SerialNums, Initial, NodePrefs, inpout2, InpOut, User, terminal, SystemPrefs, Message_Editor, fileTrans, FileTrans2, HUtilsTWO, HermesUtils, notification, PPCToolbox, Processes, EPPC, AppleEvents, HUtils3;
...
300:
	...
	begin
		...
		Date2Secs(intSystRec.lastMaint, tempLong);
		GetDateTime(tl2);
		if (intSystRec.lastMaint.year <> 0) and (tl2 < tempLong) and (length(theNodes[1]^.mySystNode^.realSerial) = 0) then
			pInvokeInvalidSerial(curGlobs^.expiredJump);
	...
	begin
		...
		begin
			...
			begin
				...
				else if (BoardSection = Logon) and (serKe = char(6)) and (LogonStage = Phone) then
				begin
					yaba := mySystNode^.realSerial;
					if length(yaba) > 0 then
					begin
						yaba := concat(HERMES_VERSION, copy(yaba, 1, 8));
						result := AsyncMWrite(outputRef, length(yaba), pointer(ord4(@yaba) + 1));
					end;
				end
			...
			case BoardSection of
				...
				begin
					...
					case LogonStage of
						...
						begin
							GetDateTime(templong);
							if ((tempLong - myUsers^^[0].first) < 2678400) or (length(intSystRec.realSerial) > 0) then
							begin
							...
							begin
								ProblemRep('Sorry, cannot run without a serial number for more than thirty days.');
								HangUpandReset;
							end;
						end;
						Stats: 
						...
						begin
							...
							if length(intSystRec.realSerial) > 0 then
								tempstring := copy(intSystRec.realSerial, 1, 8)
							else
								tempstring := concat('Un', 're', 'gi', 'st', 'er', 'ed');
							bufferIt(concat('System is      : Hermes v', HERMES_VERSION, ' #', tempstring), true, 0);
