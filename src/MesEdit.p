...
{$IFC COPYPROTECT}
		myHUtils2 := CharsHandle(Get1Resource('CODE', 10));
		if myHUtils2 <> nil then
		begin
			LENGTH := GetHandleSize(handle(myHUtils2));
			cksm := 0;
			for i := 0 to (LENGTH - 1) do
				cksm := cksm + ord(myHUtils2^^[i]);
			if cksm <> theHU2cksm then
				screwUp := true;
		end
		else
			screwUp := true;
		myHUtils2 := CharsHandle(Get1Resource('CODE', 12));
		if myHUtils2 <> nil then
		begin
			LENGTH := GetHandleSize(handle(myHUtils2));
			cksm := 0;
			for i := 0 to (LENGTH - 1) do
				cksm := cksm + ord(myHUtils2^^[i]);
			if cksm <> theH4cksm then
				screwUp := true;
		end
		else
			screwUp := true;
		myHUtils2 := CharsHandle(Get1Resource('CODE', 3));
		if myHUtils2 <> nil then
		begin
			LENGTH := GetHandleSize(handle(myHUtils2));
			cksm := 0;
...
{$D+}
	...
	begin
		with curglobs^ do
		begin
			if not screwUp and (curBase <> nil) then
			begin
