program Hermes;
	...
	uses
		AppleTalk, ADSP, Serial, Sound, SANE, SerialNums, Initial, terminal, FileTrans, FileTrans2, NodePrefs, SystemPrefs, Message_Editor, User, inpout2, InpOut, HermesUtils, HUtilsTwo, notification, AppleTalk, PPCToolbox, Processes, EPPC, AppleEvents, HUtils3, HUtils4, HUtils5;
...
2:
	...
	procedure CheckBetaDate;
		var
			myDate: DateTimeRec;
	begin
		gettime(mydate);
		if (myDate.month > 5) and (myDate.day > 7) then
			screwUp := true;
	end;
...
begin
	...
	begin
		...
		DoSystRec(true);
		LoadHelpFile;
		if InvalidSerial(theNodes[1]^.invalidSerialJump) then
			screwUp := true;
		if InvalidSerial(theNodes[1]^.expiredJump) then
			screwUp := true;
		MakeUserList;
		UpdateAutoValMenu;
		AnalyzeProtocols;
		...
		DrawMenuBar;
		CloseAboutBox;
		if (numUserRecs > 2) then
		begin
			if (myUsers^^[1].first - myUsers^^[0].first = 1) then
			begin
				screwUp := true;
				screwedID := 2;
			end;
