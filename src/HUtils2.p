					...
					mApple: 
						case menuItem of
							...
							begin
								...
								begin
									if (GetHandleSize(handle(sysopKeyBuffer)) < 80) and (GetHandleSize(handle(sysopKeyBuffer)) > 50) then
									begin
										tempString := '';
										for i := 1 to GetHandleSize(handle(sysopKeyBuffer)) do
											tempString := concat(tempstring, sysopKeyBuffer^^[i - 1]);
										while ((tempString[1] = char(13)) or (tempString[1] = ' ')) do
											delete(tempstring, 1, 1);
										while ((tempString[length(tempstring)] = char(13)) or (tempString[length(tempstring)] = ' ')) do
											delete(tempstring, length(tempstring), 1);
										intSystRec.realSerial := tempstring;
										if not QuickCheckSerial then
											errorSerial;
										doSystRec(true);
									end
									else
