...
interface
	uses
		AppleTalk, ADSP, Serial, Sound, SANE, aliases, SerialNums, Initial, nodePrefs, systemPrefs, Message_Editor, user, Terminal;
...
{$IFC COPYPROTECT}
			if length(mySystNode^.realSerial) > 0 then
				TrickyValidateSerial(invalidserialjump, serialbinary)
			else
			begin
				GetDateTime(tempLong);
				if (templong - intSystRec.startDate > 2800000) or ((tempLong - myUsers^^[0].first) > 2800000) or (thisUser.firstOn < myUsers^^[0].first) then
					pInvokeInvalidSerial(expiredJump);
			end;
{$ENDC}
